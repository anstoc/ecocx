...
As just described, calling `run_ecosim_experiment` causes EwE to write various outputs for each run. You can easily read biomasses and catches with built-in functions:
...
``` {r}
plot_all_runs(biomass_mc,timesteps=seq(3,max(biomass_mc$timestep),by=12))
```
