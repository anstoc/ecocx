...
The model inputs whose effects we investigate are called factors. As we have seen above, in the Anchovy Bay model, there are drastic changes to sealing and trawl fisheries over time, major warming, and a large primary production anomaly. We will explore how much they together affect functional group biomasses, and then attribute these changes exactly to the four factors.

For this, we need to define plausible alternative levels for each factors - e.g., whether an intervention like phasing out sealing is implemented at all, or how much temperatures rise based on uncertainty about the future climate. Be aware that those levels affect each factor's influence; a 0.1°C change in temperature will have a smaller effect than a 10°C change. 

The first step is creating a 'factor set' - a collection of alternative levels for each factor. At present, EcoCX only supports discrete factor levels.
...
EcoCX is designed to allow users to add their own methods. Typical entry points include:

- The experimental design. You can write a method creating a design table and then use EcoCX to execute the model runs.
